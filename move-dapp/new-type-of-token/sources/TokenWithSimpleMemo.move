address TokenCreator{
   ...
   module TokenWithSimpleMemo {
      ...
      // struct TheTokenWithSimpleMemo has copy, drop, store {
      //    token: TheToken,
      //    memo: vector<u8>
      // }

      struct TheToken has copy, drop, store { }

      public(script) fun init(account: signer) {
            Token::register_token<TheToken>(&account, 3);
            Account::do_accept_token<TheToken>(&account);
      }

      public(script) fun mint(account: signer, amount: u128) {
         let token = Token::mint<TheToken>(&account, amount);
         Account::deposit_to_self<TheToken>(&account, token)
      }
   }
}